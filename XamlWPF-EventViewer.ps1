...
# Create some variables that correspond to the Controls
...
#Sending Data to Controls - On_Click function is only ran when the event is triggered. Create variab;e for input to send to powershell Check-event function
$start.Add_Click({
$event=$eid.text
$computer = $comp.text
$da = $date.SelectedDate
if($event -eq "" -or $computer -eq "" -or $da -eq $null){
[System.Windows.MessageBox]::Show("Please make sure all values are entered", "Missing Values")
}Else{Check-Event $event $computer $d}
...
<# the following code will pop up a message box to validate the errors entered, must find correct place for it!
If($event -eq "" -or $computer -eq "" -or $da -eq $null){
[System.Windows.MessageBox]::Show("Please make sure all values are entered", "Missing Values")
}Else{Check-Event $event $computer $d}
#>